...
#include "GameplayDebuggerCategory_Order.h"
...
FGameplayDebuggerCategory_Order::FGameplayDebuggerCategory_Order()
{
	MaxQueries = 5;
	MaxItemTableRows = 10;
	ShownQueryIndex = 0;
...
#if USE_EQS_DEBUGGER
	SetDataPackReplication<FOrderQueryRepData>(&DataPack);
#endif
}

void FGameplayDebuggerCategory_Order::CollectData(APlayerController* OwnerPC, AActor* DebugActor)
{
	IRTSObjectInterface* rtsobject = Cast<IRTSObjectInterface>(DebugActor);
	if (rtsobject == nullptr)
	{
		return;
	}

	const URTSOrder* order = rtsobject->GetCurrentOrder();
	if (!IsValid(order))
	{
		return;
	}

	order->DescribeSelfToGameplayDebugger(this);
	...
	TArray<FEQSDebugger::FEnvQueryInfo> AuthQueryData = QueryManager->GetDebugger().GetAllQueriesForOwner(order);
...
void FGameplayDebuggerCategory_Order::DrawData(APlayerController* OwnerPC, FGameplayDebuggerCanvasContext& CanvasContext)
{
	for (const TPair<FString, FString>& keyval : StringPropertyMap)
	{
		CanvasContext.Printf(TEXT("%s: {yellow}%s"), *keyval.Key, *keyval.Value);
	}
...
FDebugRenderSceneProxy* FGameplayDebuggerCategory_Order::CreateDebugSceneProxy(const UPrimitiveComponent* InComponent, FDebugDrawDelegateHelper*& OutDelegateHelper)
{
#if USE_EQS_DEBUGGER
...
void FGameplayDebuggerCategory_Order::OnDataPackReplicated(int32 DataPackId)
{
...
RTS_PROJECT_API TSharedRef<FGameplayDebuggerCategory> FGameplayDebuggerCategory_Order::MakeInstance()
{
	return MakeShareable(new FGameplayDebuggerCategory_Order());
}
...
int32 FGameplayDebuggerCategory_Order::DrawLookedAtItem(const EQSDebug::FQueryData& QueryData, APlayerController* OwnerPC, FGameplayDebuggerCanvasContext& CanvasContext) const
{
	UWorld* World = CanvasContext.GetWorld();
...
void FGameplayDebuggerCategory_Order::DrawDetailedItemTable(const EQSDebug::FQueryData& QueryData, int32 LookedAtItemIndex, FGameplayDebuggerCanvasContext& CanvasContext) const
{
...
void FGameplayDebuggerCategory_Order::DrawDetailedItemRow(const EQSDebug::FItemData& ItemData, const TArray<uint8>& TestRelevancy, float MaxScore, FGameplayDebuggerCanvasContext& CanvasContext) const
{
...
void FGameplayDebuggerCategory_Order::FOrderQueryRepData::Serialize(FArchive& Ar)
{
	Ar << QueryDebugData;
}
#endif
